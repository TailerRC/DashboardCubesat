...
// =============================================================================
// Sensores ACTIVOS   : GPS NEO-7M · BME280 · GUVA-S12SD
// Sensores PENDIENTES: INA219 · MPU6050 · MQ135 · NRF24L01
...
//   - MPU6050            por ElectronicCats          (PENDIENTE)
...
//     INA219  → 0x40          [PENDIENTE — conectar en paralelo al mismo bus]
//     MPU6050 → 0x68          [PENDIENTE — conectar en paralelo al mismo bus]

// ── ADC (GUVA-S12SD) — ACTIVO ────────────────────────────────────────────────
#define UV_PIN    4      // Entrada analógica, solo lectura

// ── ADC (MQ135) — PENDIENTE ──────────────────────────────────────────────────
#define MQ135_PIN 34     // Input-only pin (resistente a 3.3V). Descomentar al conectar.
...
void setup() {
  ...
  // ── GPS UART2 (pines originales) ───────────────────────────────────────────
  gpsSerial.begin(GPS_BAUD, SERIAL_8N1, RXD2, TXD2);

  // ── I2C Bus (pines originales) ─────────────────────────────────────────────
  ...
  if (!bme.begin(0x76, &Wire)) {
    if (!bme.begin(0x77, &Wire)) {
      Serial.println(F("ERROR: BME280 no encontrado. Revisa cableado I2C (SDA=19, SCL=18)."));
    }
  }

  // ── INA219 (I2C 0x40) — PENDIENTE ─────────────────────────────────────────
  // Conectar: SDA=19, SCL=18 (mismo bus I2C). Descomentar al conectar:
  ...
  // Conectar en Pin 34 (input-only). Descomentar al conectar:
...
void imprimirUV() {
  ...
  float voltajeUV = (uvValorAnalogico / 4095.0) * 3.3;
...
// ─────────────────────────────────────────────────────────────────────────────
// TÓPICO 1: ambiental
// ACTIVO   : BME280 (temp · hum · presión relativa) · GUVA-S12SD (UV)
// PENDIENTE: MQ135 → co2_ppm=0.0 hasta conectar (ADC Pin 34)
// ─────────────────────────────────────────────────────────────────────────────
void publicarAmbiental() {
  ...
  float pres_rel = pres_abs - presBase;
  ...
  // co2_ppm = /* fórmula de calibración RZERO */;
...
//            INA219 usa I2C SDA=19, SCL=18, addr 0x40
// ─────────────────────────────────────────────────────────────────────────────
void publicarSatelite() {
  ...
  sens["v"] = 3;  // Actualizar al conectar más sensores (máx 7)
  sens["total"] = 7;
  sens["hace_seg"] = 0.0;
...
// PENDIENTE: MPU6050 (I2C SDA=19, SCL=18, addr 0x68)
// Todos los valores = 0.0 hasta conectar el sensor
// ─────────────────────────────────────────────────────────────────────────────
void publicarOrientacion3D() {
  ...
  // float pitch = atan2f(accel_y/9.81f, sqrtf(powf(accel_x/9.81f,2)+powf(accel_z/9.81f,2))) * 180.0f/PI;
  // float roll  = atan2f(-(accel_x/9.81f), accel_z/9.81f) * 180.0f/PI;
  ...
  JsonObject data = doc.createNestedObject("data");
  // Reemplazar 0.0 con valores reales al conectar MPU6050:
  data["cabeceo_deg"]["v"]   = 0.0; data["cabeceo_deg"]["hace_seg"]  = 0.0;
  data["balanceo_deg"]["v"]  = 0.0; data["balanceo_deg"]["hace_seg"] = 0.0;
  data["accel_x"]["v"]       = 0.0; data["accel_x"]["hace_seg"]      = 0.0;
  data["accel_y"]["v"]       = 0.0; data["accel_y"]["hace_seg"]      = 0.0;
  data["accel_z"]["v"]       = 0.0; data["accel_z"]["hace_seg"]      = 0.0;
  ...
  Serial.printf("[TX ORI] pkt#%d (MPU6050 pendiente — I2C addr 0x68)\n", pkt_ori);
}

// ─────────────────────────────────────────────────────────────────────────────
// TÓPICO 5: mision
// ACTIVO   : altitud del GPS · uptime del ESP32
// PENDIENTE: cabeceo/balanceo del MPU6050 (I2C SDA=19, SCL=18, addr 0x68)
// ─────────────────────────────────────────────────────────────────────────────
void publicarMision() {
  pkt_mis++;
  ...
  // MPU6050 — PENDIENTE (I2C addr 0x68). Descomentar al conectar:
  data["cabeceo_deg"]["v"]           = 0.0; data["cabeceo_deg"]["hace_seg"]  = 0.0;
  data["balanceo_deg"]["v"]          = 0.0; data["balanceo_deg"]["hace_seg"] = 0.0;
