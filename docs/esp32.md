...
#include <MPU6050.h>
#include <SPI.h>               // SPI Bus para NRF24L01
#include <RF24.h>              // NRF24L01 (ACTIVO)
...
#define INA219_ADDR  0x40   // [ACTIVO]   INA219 en 0x40
...
// =============================================================================
const char* WIFI_SSID      = "ELIAS 2.4GHZ";
const char* WIFI_PASSWORD  = "13141516";
...
void setup() {
  ...
  // ── INA219 — ACTIVO (I2C 0x40) ────────────────────────────────────────────
  if (!ina219.begin()) {
    Serial.println(F("❌ INA219 no detectado en 0x40. Revisa cableado."));
  } else {
    ina219Ok = true;
    Serial.println(F("✅ INA219 Detectado en 0x40"));
  }
  ...
  Serial.print(F(" - INA219     : ")); Serial.println(ina219Ok ? F("ACTIVO (I2C 0x40)") : F("NO DETECTADO"));
  Serial.print(F(" - NRF24L01   : ")); Serial.println(nrfOk ? F("ACTIVO (SPI HSPI)") : F("NO DETECTADO"));
  Serial.println(F("==========================================\n"));

  // ── NRF24L01 (SPI HSPI) — ACTIVO ──────────────────────────────────────────
  SPI.begin(NRF_SCK, NRF_MISO, NRF_MOSI, NRF_CSN);
  if (!radio.begin()) {
    Serial.println(F("❌ NRF24L01 no detectado en SPI HSPI. Revisa cableado."));
  } else {
    nrfOk = true;
    radio.setChannel(1);
    radio.setPALevel(RF24_PA_LOW);
    Serial.println(F("✅ NRF24L01 Detectado e inicializado en canal 1"));
  }
...
void publicarSatelite() {
  ...
  // INA219 — ACTIVO (I2C SDA=21, SCL=22, addr 0x40)
  ...
  Serial.printf("[TX SAT] pkt#%d temp_mcu=%.1f°C uptime=%ds volt=%.2fV curr=%.1fmA\n", pkt_sat, temp_mcu, uptime, voltaje_v, corriente_ma);
}
...
void publicarUbicacion() {
  ...
  data["calidad_senal"]["v"]      = gps.satellites.isValid() ? min((uint32_t)10, (uint32_t)gps.satellites.value()) : 0;
...
void publicarComunicacion() {
  ...
  char logText[64];
  if (nrfOk) {
    sprintf(logText, "PKT#%03d - CMD:ACK - OBC ONLINE (WiFi + RF)", pkt_com);
  } else {
